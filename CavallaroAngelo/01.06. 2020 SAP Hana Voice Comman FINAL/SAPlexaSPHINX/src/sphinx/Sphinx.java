...
import guiSAP.DialogGoodsMoved;
...
public class Sphinx {
								...
								if (listenbool || selectOrderNow) {
									...
									Display.getDefault().syncExec(new Runnable() {
										public void run() {
											...
											statusSAPIndicator
													.setText(Sphinx.getSAPlexaStatus(listenbool, selectOrderNow));
											try {
	...
	public void makeDecision(String speech, List<WordResult> speechWords) {
		...
		case "okay": {
			...
			if (selectOrderNow) {
				...
				openPickOutMonitor((Order) list.toArray()[Integer.parseInt(selectedOrderID)]);
			} else if (list.size() > 1) {
				selectOrderNow = true;
			} else if (list.size() == 1) {
				selectOrderNow = false;
				selectedOrderID = "0";
				openPickOutMonitor((Order) list.toArray()[0]);
			}
		...
		case "book order": {
			if (list.size() == 1) {
				try {
					moveGoodsToStock((Order) list.toArray()[0]);
				} catch (JCoException e) {
					e.printStackTrace();
				}
			} else {
	...
	private void openPickOutMonitor(Order o) {
	...
	public void moveGoodsToStock(Order o) throws JCoException {
		SAPConnection sc = new SAPConnection();
		String matdoc = sc.moveGoodsToStock(o);
